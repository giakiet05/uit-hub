...
export function registerRoomTools(server) {
    server.tool("room_get_availability", "Tra cứu phòng trống.", {
        token: z.string().describe("Bearer token"),
        ...QueryRoomsAvailabilitySchema
    }, async ({ token, date, start, end }) => {
        const result = await callFakeServer("room_get_availability", "GET", "/rooms/availability", token, undefined, { date, start, end });
        return formatMcpResponse(result);
    });
    server.tool("room_plan_meeting", "Đề xuất phòng họp phù hợp từ danh sách phòng trống.", {
        token: z.string().describe("Bearer token"),
        ...QueryRoomsAvailabilitySchema,
        ...
        let rooms = Array.isArray(result.data) ? result.data : (result.data?.rooms || []);
        if (capacity) {
            rooms = rooms.filter((r) => r.capacity && r.capacity >= capacity);
        }
        if (building) {
        ...
        if (equipment && equipment.length > 0) {
            rooms = rooms.filter((r) => {
                ...
                return equipment.every((e) => roomEqs.includes(e));
            });
        }
