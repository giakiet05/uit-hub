...
export function registerCourseTools(server) {
    server.tool("course_get_deadlines", "Lấy danh sách các hạn chót bài tập.", {
    ...
    server.tool("course_get_materials", "Lấy tài liệu của một môn học.", {
        token: z.string().describe("Bearer token"),
        ...PathCourseIdSchema
    ...
    server.tool("course_get_assignments", "Lấy danh sách bài tập của môn học.", {
        token: z.string().describe("Bearer token"),
        ...PathCourseIdSchema
    }, async ({ token, courseId }) => {
    ...
    server.tool("course_create_submission", "Nộp bài tập (text, link hoặc file).", {
        token: z.string().describe("Bearer token"),
