...
export function registerStudentTools(server) {
    // --- Query Tools ---
    server.tool("student_get_profile", "Lấy thông tin cá nhân của sinh viên.", { token: z.string().describe("Bearer token") }, async ({ token }) => {
        const result = await callFakeServer("student_get_profile", "GET", "/student/profile", token);
        return formatMcpResponse(result);
    });
    server.tool("student_get_schedule", "Xem lịch học học kỳ.", { token: z.string().describe("Bearer token"), ...QueryYearSemesterSchema }, async ({ token, year, semester }) => {
        const result = await callFakeServer("student_get_schedule", "GET", "/student/schedule", token, undefined, { year, semester });
        return formatMcpResponse(result);
    });
    server.tool("student_get_exam_schedule", "Xem lịch thi học kỳ.", { token: z.string().describe("Bearer token"), ...QueryYearSemesterSchema }, async ({ token, year, semester }) => {
        const result = await callFakeServer("student_get_exam_schedule", "GET", "/student/schedule/exam", token, undefined, { year, semester });
        return formatMcpResponse(result);
    });
    server.tool("student_get_score", "Xem bảng điểm.", { token: z.string().describe("Bearer token") }, async ({ token }) => {
        const result = await callFakeServer("student_get_score", "GET", "/student/score", token);
        return formatMcpResponse(result);
    });
    server.tool("student_get_tuition_fee", "Tra cứu học phí.", { token: z.string().describe("Bearer token") }, async ({ token }) => {
        const result = await callFakeServer("student_get_tuition_fee", "GET", "/student/lookup/tuitionfee", token);
        return formatMcpResponse(result);
    });
    server.tool("student_get_insurance", "Tra cứu thông tin bảo hiểm y tế.", { token: z.string().describe("Bearer token") }, async ({ token }) => {
        const result = await callFakeServer("student_get_insurance", "GET", "/student/insurance", token);
        return formatMcpResponse(result);
    });
    server.tool("student_get_courses", "Lấy danh sách các môn học.", { token: z.string().describe("Bearer token") }, async ({ token }) => {
        const result = await callFakeServer("student_get_courses", "GET", "/student/courses", token);
        return formatMcpResponse(result);
    });
    server.tool("student_get_training_points", "Tra cứu điểm rèn luyện.", { token: z.string().describe("Bearer token") }, async ({ token }) => {
        const result = await callFakeServer("student_get_training_points", "GET", "/student/training-points", token);
        return formatMcpResponse(result);
    });
    server.tool("student_get_survey_form", "Lấy danh sách form khảo sát sinh viên cần làm.", { token: z.string().describe("Bearer token") }, async ({ token }) => {
        const result = await callFakeServer("student_get_survey_form", "GET", "/student/survey-form", token);
        return formatMcpResponse(result);
    });
    // --- Composite Tools ---
    server.tool("student_get_remaining_credits", "Tính số tín chỉ còn thiếu để tốt nghiệp.", {
    ...
    server.tool("student_create_transcript_registration", "Đăng ký in bảng điểm.", {
        token: z.string().describe("Bearer token"),
    ...
    server.tool("student_create_tuition_extend", "Xin gia hạn nộp học phí.", {
        token: z.string().describe("Bearer token"),
    ...
    server.tool("student_create_monthly_parking", "Đăng ký vé xe tháng.", {
        token: z.string().describe("Bearer token"),
    ...
    server.tool("student_create_graduate_registration", "Đăng ký xét tốt nghiệp.", {
        token: z.string().describe("Bearer token"),
        year: z.number().int().min(1900),
        semester: z.number().int().min(1),
        email: z.string().email(),
    ...
    server.tool("student_create_graduation_thesis_registration", "Đăng ký khóa luận tốt nghiệp.", {
        token: z.string().describe("Bearer token"),
