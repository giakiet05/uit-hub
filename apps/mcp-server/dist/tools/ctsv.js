...
export function registerCtsvTools(server) {
    ...
    server.tool("ctsv_create_bank_loans", "Đăng ký xác nhận vay vốn ngân hàng.", {
        token: z.string().describe("Bearer token"),
    ...
    server.tool("ctsv_create_training_point_confirm", "Đăng ký giấy xác nhận điểm rèn luyện.", {
        token: z.string().describe("Bearer token"),
        language: z.enum(["VI", "EN"]),
        note: z.string().optional()
    }, async (args) => {
    ...
    server.tool("ctsv_create_language_certificate", "Nộp chứng chỉ ngoại ngữ.", {
        token: z.string().describe("Bearer token"),
