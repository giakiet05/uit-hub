...
func NewLoggerEventHandler(ctx context.Context, bus *bus.EventBus, logger *slog.Logger) *LoggerEventHandler {
	if logger == nil {
		logger = slog.Default()
	}
...
func (l *LoggerEventHandler) Start() error {
	...
	ch := l.eventBus.Subscribe(bus.TopicLifecycle)
	if ch == nil {
		cancel()
		return nil
	}
...
func (l *LoggerEventHandler) Stop() {
	...
	if ch != nil && l.eventBus != nil {
		l.eventBus.Unsubscribe(bus.TopicLifecycle, ch)
	}
}

func (l *LoggerEventHandler) loop(ctx context.Context, ch bus.EventChan) {
	for {
		select {
		case <-ctx.Done():
			return
...
func (l *LoggerEventHandler) handleEvent(ctx context.Context, event bus.Event) {
	if event == nil {
		return
	}
