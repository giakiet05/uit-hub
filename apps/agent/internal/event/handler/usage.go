...
func NewUsageEventHandler(ctx context.Context, tracker UsageTracker, bus *bus.EventBus, logger *slog.Logger) *UsageEventHandler {
	if logger == nil {
		logger = slog.Default()
	}
...
func (u *UsageEventHandler) Start() error {
	...
	ch := u.eventBus.Subscribe(bus.TopicLifecycle)
	if ch == nil {
		cancel()
		return nil
	}
...
func (u *UsageEventHandler) Stop() {
	...
	if ch != nil && u.eventBus != nil {
		u.eventBus.Unsubscribe(bus.TopicLifecycle, ch)
	}

	u.logger.Debug("Usage subscriber stopped")
}

func (u *UsageEventHandler) loop(ctx context.Context, ch bus.EventChan) {
	for {
		select {
		case <-ctx.Done():
...
func (u *UsageEventHandler) handleEvent(event bus.Event) {
	if event == nil {
		return
	}
